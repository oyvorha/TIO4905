...
def save_output(models, fixed, dynamics, station_obj):
    ...
    for i in range(len(models)):
        ...
        new_row = {'Init #stations': fixed[i].initial_size, 'No. of stations': len(fixed[i].stations)-2,
                   'No. of vehicles': len(fixed[i].vehicles), 'Objective Value': models[i][0].objVal,
                   'Vehicle capacity': fixed[i].vehicle_cap[0], 'Station capacity': fixed[i].station_cap[1],
                   'Time Horizon': fixed[i].time_horizon, 'Demand scenario': fixed[i].demand_scenario,
                   'Solution time': models[i][1], 'Gap': models[i][0].mipgap, 'weight violation': fixed[i].w_violation,
